...
namespace TTGarmentAdmin
{
    ...
    {
        private string ApiUrl = "http://nfcapi.bisplindia.in/api/Home/";
        ...
        public async Task<List<VehicleDetail>> GetVehicleList()
        {
            var result = await CallPostFunction(string.Empty, ListRegisteredVehicle);
            if (result == null || !result.Status)
            {
                return null;
            }
            ...
            {
                var memberDetail = JsonConvert.DeserializeObject<List<VehicleDetail>>(result.ResponseValue);
                return memberDetail;
            }
        }
